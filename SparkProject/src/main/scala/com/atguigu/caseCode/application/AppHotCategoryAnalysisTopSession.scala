package com.atguigu.caseCode.application

import com.atguigu.caseCode.application.AppHotCategoryAnalysisTop10Two.envData
import com.atguigu.caseCode.bean.UserVisitAction
import com.atguigu.summerframework.code.Application
import org.apache.spark.{SPARK_BRANCH, SparkContext}


object AppHotCategoryAnalysisTopSession extends Application{

  def main(args: Array[String]): Unit = {
    start("spark"){
      val sc = envData.asInstanceOf[SparkContext]
      val rdd = sc.textFile("input/Top10")
      // 求出Top10热门品类

      val rdd1 = rdd.map(line => {
        val word = line.split(",")(3)
        word.substring(0, word.length - 1)
      })
      val top10Value = rdd1.collect()
      // 因为每个task都用来做比较分析,所以需要放入到广播变量中
      val value = sc.broadcast(top10Value)

      // 求出Top热门品类,每个品牌,每个的session的点击次数

      val rdd2 = sc.textFile("input/user_visit_action.txt")
      val rdd3 = rdd2.map(line => {
        val arr = line.split("_")
        UserVisitAction(arr(0), //用户点击行为的日期
          arr(1).toLong, //用户的ID
          arr(2), //Session的ID
          arr(3).toLong, //某个页面的ID
          arr(4), //动作的时间点
          arr(5), //用户搜索的关键词
          arr(6).toLong, //某一个商品品类的ID
          arr(7).toLong, //某一个商品的ID
          arr(8), //一次订单中所有品类的ID集合
          arr(9), //一次订单中所有商品的ID集合
          arr(10), //一次支付中所有品类的ID集合
          arr(11), //一次支付中所有商品的ID集合
          arr(12).toLong //城市 id
        )
      }).filter(_.search_keyword== "null")
      val rdd4 = rdd3.filter(bean => {
        value.value.contains(bean.click_category_id.toString)
      }).map(bean=>((bean.click_category_id,bean.session_id),1))
      //  按品类和session 统计
      val rdd5 = rdd4.reduceByKey(_ + _)
        .map{
          case ((id,session),count)=>(id,(session,count))
        }
      // 将第二步的结果，按照品类分组，根据session的点击次数排序，取前十
      val rdd6 = rdd5.groupByKey()
        .mapValues(iter => iter.toList.sortBy(-_._2).take(10))
      rdd6.saveAsTextFile("output4")
    }("output4")
  }

}
